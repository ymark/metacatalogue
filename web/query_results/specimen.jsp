...
<%@page import="eu.lifewatch.core.model.SpecimenStruct"%>
...
<html>
    ...
    <body>
        ...
        <div class="results_wrapper">
            <h3>Specimen Information Found</h3>
            <table class="footable table table-bordered" >
                <thead>
                    ...
                    <th data-hide="all">Specimen Name</th>                                 
                    <th data-hide="all">Individual</th>
                    <th data-hide="all">Transformation ID</th>                      
                    <th data-hide="all">Actor</th>             
                    <th data-hide="all">Date</th>                                             
                </thead>
                <tbody>
                    <c:forEach items="${results}" var="item" varStatus="status">
                        <% Pair actors = ((SpecimenStruct)pageContext.getAttribute("item")).getActors().get(0); %>
                        <tr>
                            ...
                            <td style="text-align: left"><a href="${baseUrl}/search/browse?uri=${item.getMethodURI()}">${item.getMethodName()}</a></td>                              
                            ...
                            <td><a href="${baseUrl}/search/browse?uri=${item.getSpecimenURI()}">${item.getSpecimenName()}</a></td>                                                
                            <td><a href="${baseUrl}/search/browse?uri=${item.getIndividualURI()}">${item.getIndividualURI()}</a></td>
                            <td><a href="${baseUrl}/search/browse?uri=${item.getTransformationEventURI()}">${item.getTransformationEvent()}</a></td>                                      
                ...
                <nav>
                    <ul class="pagination">
                        <li <c:if test = "${!leftArrow}">class='disabled'</c:if> >
                            <a href="${baseUrl}/search/advanced/specimen?species=${species}&collection=${collection}&specimen=${specimen}&page=${page-1}" aria-label="Previous">
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>     
                                    <c:if test = "${loop.index <= lastPage}">
                                        <li><a href="${baseUrl}/search/advanced/specimen?species=${species}&collection=${collection}&specimen=${specimen}&page=${loop.index}">${loop.index}</a></li>
                                    </c:if>                                                                      
                                </c:otherwise>
                            </c:choose>                        
                        </c:forEach>
                        <c:if test = "${rightArrow}">
                            <li>
                                <a href="${baseUrl}/search/advanced/specimen?species=${species}&collection=${collection}&specimen=${specimen}&page=${page+1}" aria-label="Next">
