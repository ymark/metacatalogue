...
<%@page import="eu.lifewatch.core.model.StatsStruct"%>
<%@page import="eu.lifewatch.core.model.Pair"%>
<%@page import="eu.lifewatch.core.model.MorphometricsStruct"%>
...
<html>
    ...
    <body>
        ...
        <div class="results_wrapper">
            <h3>Statistical Information Found</h3>
            <table class="footable table table-bordered" >
                <thead>
                    ...
                    <th data-hide="all">Attribute Assignment Event ID</th>                                 
                    ...
                    <th data-hide="all">Specimen</th>
                    <th data-hide="all">Publication</th>
                </thead>
                <tbody>
                    <c:forEach items="${results}" var="item" varStatus="status">                    
                        <% Pair actors = ((StatsStruct)pageContext.getAttribute("item")).getActors().get(0); %>
                        <tr>
                            ...
                            <td><a href="${baseUrl}/search/browse?uri=${item.getDataEvaluationURI()}">${item.getDataEvaluation()}</a></td>                        
                            <td>${item.getDimensionUnit()}</td>
                            <td>${item.getDimensionValue()}</td>
                            <td>${item.getDimensionTypeURI()}</td>
                            ...
                            <td><a href="${baseUrl}/search/browse?uri=${item.getPublicationURI()}">${item.getPublicationName()}</a></td>             
                        <tr/>
                    </c:forEach>
                </tbody>
            </table>
                ...
                <nav>
                    <ul class="pagination">
                        <li <c:if test = "${!leftArrow}">class='disabled'</c:if> >
                            <a href="${baseUrl}/search/advanced/statistical?species=${species}&dimension=${dimension}&page=${page-1}" aria-label="Previous">
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>     
                                    <c:if test = "${loop.index <= lastPage}">
                                        <li><a href="${baseUrl}/search/advanced/statistical?species=${species}&dimension=${dimension}&page=${loop.index}">${loop.index}</a></li>
                                    </c:if>                                                                      
                                </c:otherwise>
                            </c:choose>                        
                        </c:forEach>
                        <c:if test = "${rightArrow}">
                            <li>
                                <a href="${baseUrl}/search/advanced/statistical?species=${species}&dimension=${dimension}&page=${page+1}" aria-label="Next">
