...
<html>
    ...
    <body>
        ...
        <div class="results_wrapper">
            <h3>Synonym Information Found</h3>
                        ...
                        <tr>
                            ...
                            <td>
                                <table class="table table-condensed table-bordered">
                                    ...
                                    <tbody>
                                    <c:forEach items="${item.getSynonyms()}" var="synonym">
                                        <tr>
                                            <td>
                                                <a href="${baseUrl}/search/browse?uri=${synonym.getKey().toString()}">${synonym.getValue().toString()}</a>                                            
                                            </td>
                                        </tr>
                                    </c:forEach>
                                    </tbody>
                ...
                <nav>
                    <ul class="pagination">
                        <li <c:if test = "${!leftArrow}">class='disabled'</c:if> >
                            <a href="${baseUrl}/search/advanced/synoym?species=${species}&sname=${sname}&synonym=${synonym}&page=${page-1}" aria-label="Previous">
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>     
                                    <c:if test = "${loop.index <= lastPage}">
                                        <li><a href="${baseUrl}/search/advanced/synoym?species=${species}&sname=${sname}&synonym=${synonym}&page=${loop.index}">${loop.index}</a></li>
                                    </c:if>                                                                      
                                </c:otherwise>
                            </c:choose>                        
                        </c:forEach>
                        <c:if test = "${rightArrow}">
                            <li>
                                <a href="${baseUrl}/search/advanced/synoym?species=${species}&sname=${sname}&synonym=${synonym}&page=${page+1}" aria-label="Next">
