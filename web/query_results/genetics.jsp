...
<html>
    ...
    <body>
        ...
        <div class="results_wrapper">
            <h3>Genetics Sampling Information Found</h3>
            <table class="footable table table-bordered" >
                <thead>
                    ...
                    <th data-hide="all">Sample</th> 
                    <th data-hide="all">Date</th>                                  
                    <th data-hide="all">Habitat</th>                
                    <th data-hide="all">Description</th>  
                    <th data-hide="all">Produced File</th>                  
                    <th data-hide="all">Device Type</th> 
                </thead>
                <tbody>
                    <c:forEach items="${results}" var="item" varStatus="status">                   
                        <tr>
                            ...
                            <td style="text-align: left"><a href="${baseUrl}/search/browse?uri=${item.getDeviceURI()}">${item.getDevice()}</a></td>
                            <td style="text-align: left"><a href="${baseUrl}/search/browse?uri=${item.getEcosystemURI()}">${item.getEcosystemName()}</a></td>                            
                            ...
                            <td><a href="${baseUrl}/search/browse?uri=${item.getSampleURI()}">${item.getSampleName()}</a></td>
                            <td>${item.getTimeSpan()}</td>                            
                            <td><a href="${baseUrl}/search/browse?uri=${item.getHabitatURI()}">${item.getHabitatName()}</a></td>
                            <td>${item.getDescription()}</td>
                            <td><a href="${baseUrl}/search/browse?uri=${item.getProducedFileURI()}">${item.getProducedFile()}</a></td>                         
                ...
                <nav>
                    <ul class="pagination">
                        <li <c:if test = "${!leftArrow}">class='disabled'</c:if> >
                            <a href="${baseUrl}/search/advanced/genetics?species=${species}&device=${device}&sample=${sample}&place=${place}&page=${page-1}" aria-label="Previous">
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>     
                                    <c:if test = "${loop.index <= lastPage}">
                                        <li><a href="${baseUrl}/search/advanced/genetics?species=${species}&device=${device}&sample=${sample}&place=${place}&page=${loop.index}">${loop.index}</a></li>
                                    </c:if>                                                                      
                                </c:otherwise>
                            </c:choose>                        
                        </c:forEach>
                        <c:if test = "${rightArrow}">
                            <li>
                                <a href="${baseUrl}/search/advanced/genetics?species=${species}&device=${device}&sample=${sample}&place=${place}&page=${page+1}" aria-label="Next">
