...
<%@page import="eu.lifewatch.core.model.IdentificationStruct"%>
...
<html>
    ...
    <body>
        ...
        <div class="results_wrapper">
            <h3>Identification Information Found</h3>
            <table class="footable table table-bordered" >
                <thead>
                    ...
                    <th data-hide="all">Identification References</th>
                    <th data-hide="all">Identification Event ID</th>   
                </thead>
                <tbody>
                    <c:forEach items="${results}" var="item" varStatus="status">
                        <% Pair actors = ((IdentificationStruct)pageContext.getAttribute("item")).getActors().get(0); %>
                        <tr>
                            ...
                            <td style="text-align: left"><a href="${baseUrl}/search/browse?uri=${item.getLocalityURI()}">${item.getLocalityName()}</a></td>
                            ...
                            <td><a href="${baseUrl}/search/browse?uri=<% out.print(actors.getValue().toString()); %>"><% out.print(actors.getValue().toString()); %></a></td>                                                                            
                            <td><a href="${baseUrl}/search/browse?uri=${item.getIdentificationReferencesURI()}">${item.getIdentificationReferencesName()}</a></td>                            
                            <td><a href="${baseUrl}/search/browse?uri=${item.getIdentificationEventURI()}">${item.getIdentificationEvent()}</a></td>                        
                        <tr/>
                    </c:forEach>
                </tbody>
            </table>
                ...
                <nav>
                    <ul class="pagination">
                        <li <c:if test = "${!leftArrow}">class='disabled'</c:if> >
                            <a href="${baseUrl}/search/advanced/identification?species=${species}&place=${place}&date=${date}&actor=${actor}&individual=${individual}&page=${page-1}" aria-label="Previous">
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>     
                                    <c:if test = "${loop.index <= lastPage}">
                                        <li><a href="${baseUrl}/search/advanced/identification?species=${species}&place=${place}&date=${date}&actor=${actor}&individual=${individual}&page=${loop.index}">${loop.index}</a></li>
                                    </c:if>                                                                      
                                </c:otherwise>
                            </c:choose>                        
                        </c:forEach>
                        <c:if test = "${rightArrow}">
                            <li>
                                <a href="${baseUrl}/search/advanced/identification?species=${species}&place=${place}&date=${date}&actor=${actor}&individual=${individual}&page=${page+1}" aria-label="Next">
