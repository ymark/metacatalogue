...
<html>
    ...
    <body>
        ...
        <div class="results_wrapper">
            <h3>MicroCT Reconstruction Information Found</h3>
            <table class="footable table table-bordered" >
                <thead>
                    ...
                    <th data-hide="all">Reconstruction ID</th>                
                    <th data-hide="all">Input Name</th>               
                    <th data-hide="all">Description</th>                                                        
                </thead>
                <tbody>
                    <c:forEach items="${results}" var="item" varStatus="status">
                        <% Pair inputs = ((MicroCTReconstructionStruct)pageContext.getAttribute("item")).getInputs().get(0); %>
                        <% Pair products = ((MicroCTReconstructionStruct)pageContext.getAttribute("item")).getProducts().get(0); %>
                        <tr>
                            ...
                            <td style="text-align: left"><a href="${baseUrl}/search/browse?uri=<% out.print(inputs.getKey().toString()); %>"><% out.print(products.getValue().toString()); %></a></td>                                                        
                            ...
                            <td><a href="${baseUrl}/search/browse?uri=${item.getReconstructionURI()}">${item.getReconstruction()}</a></td>                                                
                            <td><a href="${baseUrl}/search/browse?uri=<% out.print(products.getKey().toString()); %>"><% out.print(inputs.getValue().toString()); %></a></td>       
                ...
                <nav>
                    <ul class="pagination">
                        <li <c:if test = "${!leftArrow}">class='disabled'</c:if> >
                            <a href="${baseUrl}/search/advanced/microct_reconstruction?species=${species}&specimen=${specimen}&input=${input}&page=${page-1}" aria-label="Previous">
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>     
                                    <c:if test = "${loop.index <= lastPage}">
                                        <li><a href="${baseUrl}/search/advanced/microct_reconstruction?species=${species}&specimen=${specimen}&input=${input}&page=${loop.index}">${loop.index}</a></li>
                                    </c:if>                                                                      
                                </c:otherwise>
                            </c:choose>                        
                        </c:forEach>
                        <c:if test = "${rightArrow}">
                            <li>
                                <a href="${baseUrl}/search/advanced/microct_reconstruction?species=${species}&specimen=${specimen}&input=${input}&page=${page+1}" aria-label="Next">
