...
<html>
    ...
    <body>
        ...
        <div class="results_wrapper">
            <h3>Morphometric Information Found</h3>
            <table class="footable table table-bordered" >
                <thead>
                    ...
                    <th data-hide="all">Publication Name</th>        
                </thead>
                <tbody>
                    <c:forEach items="${results}" var="item" varStatus="status">
                        <% Pair actors = ((MorphometricsStruct)pageContext.getAttribute("item")).getActors().get(0); %>
                        <tr>
                            ...
                            <td><a href="${baseUrl}/search/browse?uri=${item.getAttributeAssignmentEventURI()}">${item.getAttributeAssignmentEvent()}</a></td>                        
                ...
                <nav>
                    <ul class="pagination">
                        <li <c:if test = "${!leftArrow}">class='disabled'</c:if> >
                            <a href="${baseUrl}/search/advanced/morphometric?species=${species}&dimension=${dimension}&page=${page-1}" aria-label="Previous">
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>     
                                    <c:if test = "${loop.index <= lastPage}">
                                        <li><a href="${baseUrl}/search/advanced/morphometric?species=${species}&dimension=${dimension}&page=${loop.index}">${loop.index}</a></li>
                                    </c:if>                                                                      
                                </c:otherwise>
                            </c:choose>                        
                        </c:forEach>
                        <c:if test = "${rightArrow}">
                            <li>
                                <a href="${baseUrl}/search/advanced/morphometric?species=${species}&dimension=${dimension}&page=${page+1}" aria-label="Next">
